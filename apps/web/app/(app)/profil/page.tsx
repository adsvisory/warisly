...
import { Check, Lock, Shield, ChevronRight, Languages } from "lucide-react";
  ...
  return (
    <div className="pb-8">
        ...
        {!verified && (
          <>
            <form action={startEkycAction} className="mt-4">
              <button className="inline-flex items-center gap-2 rounded-xl bg-tinta px-5 py-3 font-sans text-sm font-semibold text-ink-text hover:bg-tinta-hover">
                {t("verifyBtn")}
              </button>
            </form>
      ...
      <form action={signOut} className="mt-8">
        <button className="font-sans text-sm font-medium text-emas hover:text-emas-ink">{t("signOut")}</button>
      </form>
    </div>
  );
}
