# IMPLEMENTATION BRIEF: Schema Foundation + RLS
## Surface: backend
## Brief: #1
## Phase: 1 (MVP)
## Depends on: #0
## Blocks: #2, #4a, #5a, #6a, #7a, #9a
## Parallel with: None

### CONTEXT
The data spine: core enums, the owner profile, global settings, the audit/transparency event log, and the external-call observability log — all with Row-Level Security from the first migration.

### NON-NEGOTIABLE CHECK
No credential columns, no money fields, no provider-auth data. RLS is enabled on every table. Owners can read only their own profile and their own events. `wrs_api_log` is fully internal (no user policy → only service-role can read). `wrs_settings` is read-only to authenticated users; writes only via service-role. This is the permission spine for "now vs after death" enforced later via `release_state`.

### PRE-FLIGHT CHECKS
- [ ] Brief #0 done (Supabase project linked, `supabase/migrations/` exists).
- [ ] `pgcrypto` available (Supabase default → `gen_random_uuid()` works).
- [ ] Confirm no existing `wrs_` tables (this is the first migration).
...
### VERIFICATION
- [ ] Migration applies clean (`supabase db push` or SQL editor), no errors.
- [ ] As an authenticated test user: `select * from wrs_owners` returns only their own row after one is inserted; inserting a row with a different `id` is rejected by RLS.
- [ ] `select * from wrs_api_log` as an authenticated (non-service) user returns **zero rows** (deny-all proven).
- [ ] `select * from wrs_settings` as authenticated returns the 6 seeded rows; an `update` attempt is rejected.
- [ ] Updating a `wrs_owners` row bumps `updated_at`.
